...
import { conciseCatcher, JsShell, zxs } from "../apputil-deno.mjs";

const yargsInst = yargs(Deno.args).
  strictOptions().
...
Command files are JSON of lists of objects with these elements:
  ...
  demandCommand(1).
  version();
const yargsDict = yargsInst.parse();
const progName = yargsDict._[0].replace(/^.*[\\/]/, "");  // eslint-disable-line no-unused-vars
...
conciseCatcher(function(cmdFiles, cwd, r0, out, err, envMap) {
    ...
    const jsShells = cmdFiles.map(cmdFile => new JsShell(
      cmdFile, JSON.parse(Deno.readTextFileSync(cmdFile)), cwd, envMap, true, Deno.env.toObject())
    );
